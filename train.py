...
def collate_for_training(batch_images, batch_input_ids, bos_id=101):
    ...
    Returns: images, captions_seq, dec_input, dec_target (numpy arrays)
    """
...
def train(cfg, keep_small=False):
    ...
    model = models['full_model']         # expects [images_seq, captions_seq, dec_input]
    ...
    # 5) Training step (no @tf.function for easier debugging)
    def train_step(images_np, captions_seq_np, dec_input_np, dec_target_np):
        ...
        with tf.GradientTape() as tape:
            ...
            # IMAGE LOSS: encode true last image and compute MSE (stop gradient on target features)
            last_images = images[:, -1, :, :, :]  # (B, H, W, C)
            target_img_feat = tf.stop_gradient(visual_enc(last_images, training=False))  # (B, feat_dim)
    ...
    for epoch in range(epochs):
        ...
        # save weights each epoch (Keras-compatible .weights.h5)
        model.save_weights(f"{ckpt_prefix}_epoch{epoch+1}.weights.h5")
        print(f"Saved checkpoint: {ckpt_prefix}_epoch{epoch+1}.weights.h5")
